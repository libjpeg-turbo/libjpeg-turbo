...
typedef struct {
  ...
  void (*AC_first_prepare) (const JCOEF *block,
                            ...
                            int Al, JABSCOEF *values, size_t *zerobits);
  /* Pointer to routine to prepare data for encode_mcu_AC_refine() */
  int (*AC_refine_prepare) (const JCOEF *block,
                            const int *jpeg_natural_order_start, int Sl,
                            int Al, JABSCOEF *absvalues, size_t *bits);
...
METHODDEF(void) encode_mcu_AC_first_prepare
  (const JCOEF *block, const int *jpeg_natural_order_start, int Sl, int Al,
   JABSCOEF *values, size_t *zerobits);
...
METHODDEF(int) encode_mcu_AC_refine_prepare
  (const JCOEF *block, const int *jpeg_natural_order_start, int Sl, int Al,
   JABSCOEF *absvalues, size_t *bits);
...
#define COMPUTE_ABSVALUES_AC_FIRST(Sl) { \
  for (k = 0; k < Sl; k++) { \
    ...
    temp2 ^= temp; \
    values[k] = (JABSCOEF)temp; \
    values[k + DCTSIZE2] = (JABSCOEF)temp2; \
...
encode_mcu_AC_first_prepare(const JCOEF *block,
                            const int *jpeg_natural_order_start, int Sl,
                            int Al, JABSCOEF *values, size_t *bits)
{
...
encode_mcu_AC_first(j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  int Al = cinfo->Al;
  JABSCOEF values_unaligned[2 * DCTSIZE2 + 15];
  JABSCOEF *values;
  const JABSCOEF *cvalue;
  size_t zerobits;
...
#ifdef WITH_SIMD
  cvalue = values = (JABSCOEF *)PAD((JUINTPTR)values_unaligned, 16);
#else
...
#define COMPUTE_ABSVALUES_AC_REFINE(Sl, koffset) { \
  ...
  for (k = 0; k < Sl; k++) { \
    ...
    absvalues[k] = (JABSCOEF)temp; /* save abs value for main pass */ \
    if (temp == 1) \
...
encode_mcu_AC_refine(j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  int Sl = cinfo->Se - cinfo->Ss + 1;
  int Al = cinfo->Al;
  JABSCOEF absvalues_unaligned[DCTSIZE2 + 15];
  JABSCOEF *absvalues;
  const JABSCOEF *cabsvalue, *EOBPTR;
...
#ifdef WITH_SIMD
  cabsvalue = absvalues = (JABSCOEF *)PAD((JUINTPTR)absvalues_unaligned, 16);
#else
  /* Not using SIMD, so alignment is not needed */
