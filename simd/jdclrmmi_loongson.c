...
GLOBAL(void)
jsimd_ycc_rgb_convert_mmi (JDIMENSION out_width,
        ...
        // mmC=(10 12 14 16 ** ** ** **), mmD=(11 13 15 17 ** ** ** **)
				...
				if(col >= sizeof(MMWORD))
				{
					...
					if(col > 0)
					{
						...
						inptr1 += sizeof(MMWORD);
						inptr2 += sizeof(MMWORD);
						outptr += RGB_PIXELSIZE*sizeof(MMWORD);
					}
					else
					break;
				}
				else
				{
					...
					"5:	nop	\r\n"				//end
					:"=m"(*outptr)
					:"f"(mmA),"f"(mmC),"f"(mmE),"r"(col_tmp),"r"(outptr)
				...
				if(col >= sizeof(MMWORD)){
					...
					"3:li	%1,0\r\n"			//end
					:"=m"(*outptr),"=r"(col)
					:"f"(mmA),"f"(mmC),"f"(mmD),"f"(mmH),"r"(col),"r"(outptr)
					:"$f4","$f6","$8","$9","$10","memory"
					);
					break;
				}
