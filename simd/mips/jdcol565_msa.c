...
void
yuv_rgb565_convert_msa (JSAMPROW p_in_y, JSAMPROW p_in_cb, JSAMPROW p_in_cr,
                        JSAMPROW p_rgb, JDIMENSION out_width)
{
  ...
  for (col = num_cols_mul_16; col--;) {
    ...
    CALC_G4_FRM_YUV(y_h0, y_h1, cb_h0, cb_h1, cr_h0, cr_h1, out_g0, out_g1);

    UNPCK_SH_SW(cr_h0, cr_w0, cr_w1);
    UNPCK_SH_SW(cr_h1, cr_w2, cr_w3);
    CALC_R4_FRM_YUV(y_h0, y_h1, cr_w0, cr_w1, cr_w2, cr_w3, out_r0, out_r1);
...
{
  while (--num_rows >= 0) {
    yuv_rgb565_convert_msa(input_buf[0][input_row], input_buf[1][input_row],
