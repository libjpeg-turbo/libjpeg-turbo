                                         ...
                                         c_derived_tbl *actbl)
{
   ...
   * Neon; else we do some of the work using Neon but the rest on demand using
   * scalar code.
   */
  if (non_zero_coefficients > 8) {
    ...
    /* Narrow leading-zero count to 8-bit. */
    ...
    /* diff = abs(coef) ^ sign(coef). (No-op for positive coefficients.) */
