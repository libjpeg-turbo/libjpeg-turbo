...
static uint8_t jsimd_h2_downsample_consts[] = {
                                ...
                                JSAMPARRAY output_data)
{
  ...
  int mask_offset = 16 * ((width_in_blocks * 2 * DCTSIZE) - image_width);
  uint8x16_t expand_mask = vld1q_u8(&jsimd_h2_downsample_consts[mask_offset]);
  /* Load bias pattern alternating every pixel. */
  uint16x8_t bias = { 0, 1, 0, 1, 0, 1, 0, 1 };

  for (int outrow = 0; outrow < v_samp_factor; outrow++) {
    outptr = output_data[outrow];
    inptr = input_data[outrow];

    /* Downsample all but the last DCT block of pixels. */
    for (int i = 0; i < width_in_blocks - 1; i++) {
                                ...
                                JSAMPARRAY output_data)
{
  ...
  uint8x16_t expand_mask = vld1q_u8(&jsimd_h2_downsample_consts[mask_offset]);
  /* Load bias pattern alternating every pixel. */
  uint16x8_t bias = { 1, 2, 1, 2, 1, 2, 1, 2 };
