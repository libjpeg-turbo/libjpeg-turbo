                                    ...
                                    size_t *zerobits)
{
  ...
  for (int i = 0; i < Sl / 16; i++) {
    int16x8_t coefs1;
    coefs1 = vld1q_lane_s16(block + jpeg_natural_order_start[0], coefs1, 0);
    ...
    /* Store transformed coefs and diff values. */
  ...
  uint8x8_t bitmap_mask = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80 };
...
int jsimd_encode_mcu_AC_refine_prepare_neon(const JCOEF *block,
                                      const int *jpeg_natural_order_start,
                                      int Sl,
                                      int Al,
                                      JCOEF *absvalues,
                                      size_t *bits)
{
  ...
  for (int i = 0; i < Sl / 16; i++) {
    ...
    uint8x8_t sign_coefs1 = vmovn_u16(
                              vreinterpretq_u16_s16(vshrq_n_s16(coefs1, 15)));
    uint8x8_t sign_coefs2 = vmovn_u16(
                              vreinterpretq_u16_s16(vshrq_n_s16(coefs2, 15)));
    ...
    /* Test transformed coef values == 1 (used to find EOB position.) */
  ...
  if (remaining_coefs > 8) {
    ...
    /* Compute and store data for signbits bitmap. */
    uint8x8_t sign_coefs = vmovn_u16(
                              vreinterpretq_u16_s16(vshrq_n_s16(coefs, 15)));
  ...
  /* Construct bitmap to find EOB position: index of last coefficient == 1. */
  ...
  /* Return EOB position. */
  if (bitmap == 0) {
    /* EOB position defined to be 0 if all coefs != 1. */
    return 0;
  } else {
