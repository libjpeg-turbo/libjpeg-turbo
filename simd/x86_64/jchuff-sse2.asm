;
; Huffman Encoding (64-bit SSE2)
;
; Copyright (C) 2009-2011, 2014-2016, 2019, 2021, 2023-2025, D. R. Commander.
...
%endif
    ...
    movaps      XMMWORD [t + 16 * SIZEOF_WORD], xmm2
                                        ;C: t[i + 16] = w2[i];
    ...
    movzx       nbitsq, byte [NBITS(codeq)]
                                        ;Z:   nbits = JPEG_NBITS(code);
    movdqa      xmm3, xmm5              ;H: w3 = 48 49 50 51 52 53 54 55
    pmovmskb    tempd, xmm2             ;Z:   temp = 0;
                                        ;     temp |= ((b2[i] >> 7) << i);
    pmovmskb    put_bufferd, xmm0       ;Z:   put_buffer = 0;
                                        ;     put_buffer |= ((b0[i] >> 7) << i);
    ...
    and         code, dword [MASK_BITS(nbitsq)]
                                        ;Z:   code &= (1 << nbits) - 1;
...
%define free_bitsb  r15b
    ...
    movaps      XMMWORD [t + 32 * SIZEOF_WORD], xmm5
                                        ;E: t[32 + i] = w5[i];
...
%define code_temp  r8d
    pmovmskb    indexd, xmm5            ;Z:   index = 0;
                                        ;     index |= ((b5[i] >> 7) << i);
...
.EMIT_CODE:                             ;Z:   .EMIT_CODE:
    EMIT_QWORD  .BLOOP_COND             ;Z:   insert code, flush buffer,
                                        ;     goto .BLOOP_COND
...
.ERLOOP:                                ; .ERLOOP:
    ...
    movzx       nbits, byte [NBITS(codeq)]
                                        ;     nbits = JPEG_NBITS(code);
    lea         tempd, [nbitsq + tempq * 8]
                                        ;     temp = temp * 8 + nbits;
...
.ELOOP:                                 ; }  /* index != 0 */
    sub         td, esp                 ; t -= &t_[0];
    cmp         td, (DCTSIZE2 - 2) * SIZEOF_WORD
                                        ; if (t != 62)
    je          .EFN                    ; {
    movzx       nbits, byte [actbl + c_derived_tbl.ehufsi + 0]
                                        ;   nbits = actbl->ehufsi[0];
    mov         code, [actbl + c_derived_tbl.ehufco + 0]
                                        ;   code = actbl->ehufco[0];
    sub         free_bitsb, nbitsb      ;   if ((free_bits -= nbits) <= 0)
