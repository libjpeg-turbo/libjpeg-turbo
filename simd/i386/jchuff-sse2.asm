;
; Huffman Encoding (32-bit SSE2)
;
; Copyright (C) 2009-2011, 2014-2017, 2019, 2024-2025, D. R. Commander.
...
%endif
%define MASK_BITS(x)  NBITS((x) * 8) + \
                      (jpeg_mask_bits - EXTN(jpeg_nbits_table))
...
; wN:     xmmN treated as 8 signed 16-bit values
; wN[i]:  perform the same operation on all 8 signed 16-bit values, i = 0..7
; bN:     xmmN treated as 16 unsigned 8-bit values, or
;         mmN treated as 8 unsigned 8-bit values
...
%define %%temph  %3
    ...
    movd        mm_overflow_bits, free_bits
                                        ; overflow_bits (temp register) = free_bits;
    neg         free_bits               ; free_bits = -free_bits;
    psllq       mm_put_buffer, mm_nbits
                                        ; put_buffer <<= nbits;
    psrlq       mm_temp, mm_overflow_bits
                                        ; temp >>= overflow_bits;
    add         free_bits, 64           ; free_bits += 64;
    por         mm_temp, mm_put_buffer  ; temp |= put_buffer;
%ifidn %%temp, nbits_base
    movd        mm_save_nbits, nbits_base
                                        ; save nbits_base
%endif
    ...
    pmovmskb    nbits, mm_temp          ; nbits = 0;
                                        ; nbits |= ((b_temp[i] >> 7) << i);
...
%endif
    ...
    mov         dword [buffer + 4], nbits
                                        ; *(uint32_t)(buffer + 4) = nbits;
...
%define arg_actbl        24 + stack_offset
    ...
    PUSH        ebx
    PUSH        ebp
    movups      xmm3, XMMWORD [block + 0 * SIZEOF_WORD]
                                        ;D: w3 = xx 01 02 03 04 05 06 07
    PUSH        esi
    PUSH        edi
    movdqa      xmm0, xmm3              ;A: w0 = xx 01 02 03 04 05 06 07
    mov         frame, esp
    lea         t, [frame - (save_frame + 4)]
    movups      xmm1, XMMWORD [block + 8 * SIZEOF_WORD]
                                        ;B: w1 = 08 09 10 11 12 13 14 15
    and         t, -DCTSIZE2 * SIZEOF_WORD
                                        ; t = &t_[0]
    ...
    pinsrw      xmm3, word [block + 20 * SIZEOF_WORD], 0
                                        ;D: w3 = 20 13 06 07 14 15 -- --
    pinsrw      xmm3, word [block + 21 * SIZEOF_WORD], 5
                                        ;D: w3 = 20 13 06 07 14 21 -- --
    pinsrw      xmm3, word [block + 28 * SIZEOF_WORD], 6
                                        ;D: w3 = 20 13 06 07 14 21 28 --
    pinsrw      xmm3, word [block + 35 * SIZEOF_WORD], 7
                                        ;D: w3 = 20 13 06 07 14 21 28 35
                                        ;        (Row 3, offset 1)
    pcmpgtw     xmm4, xmm3              ;D: w4[i] = (w3[i] < 0 ? -1 : 0);
    paddw       xmm3, xmm4              ;D: w3[i] += w4[i];
    movaps      XMMWORD [t + 24 * SIZEOF_WORD], xmm3
                                        ;D: t[i + 24] = w3[i];
    pxor        xmm4, xmm4              ;D: w4[i] = 0;
    pcmpeqw     xmm3, xmm4              ;D: w3[i] = (w3[i] == 0 ? -1 : 0);

    pinsrw      xmm2, word [block + 19 * SIZEOF_WORD], 0
                                        ;C: w2 = 19 26 25 27 48 49 50 51
    pinsrw      xmm2, word [block + 33 * SIZEOF_WORD], 2
                                        ;C: w2 = 19 26 33 27 48 49 50 51
    pinsrw      xmm2, word [block + 40 * SIZEOF_WORD], 3
                                        ;C: w2 = 19 26 33 40 48 49 50 51
    pinsrw      xmm2, word [block + 41 * SIZEOF_WORD], 5
                                        ;C: w2 = 19 26 33 40 48 41 50 51
    pinsrw      xmm2, word [block + 34 * SIZEOF_WORD], 6
                                        ;C: w2 = 19 26 33 40 48 41 34 51
    pinsrw      xmm2, word [block + 27 * SIZEOF_WORD], 7
                                        ;C: w2 = 19 26 33 40 48 41 34 27
                                        ;        (Row 2, offset 1)
    pcmpgtw     xmm4, xmm2              ;C: w4[i] = (w2[i] < 0 ? -1 : 0);
    paddw       xmm2, xmm4              ;C: w2[i] += w4[i];
    movsx       code_temp, word [block]
                                        ;Z:   code_temp = block[0];
...
%macro GET_SYM_BEFORE 1
    ...
    sub         code_temp, [frame + arg_last_dc_val]
                                        ;Z:   code_temp -= last_dc_val;

    packsswb    xmm2, xmm3              ;CD: b2[i] = w2[i], b2[i + 8] = w3[i]
                                        ;    w/ signed saturation

    movdqa      xmm3, xmm5              ;H: w3 = 48 49 50 51 52 53 54 55
    pmovmskb    index_temp, xmm2        ;Z:   index_temp = 0;
                                        ;     index_temp |= ((b2[i] >> 7) << i);
    pmovmskb    index, xmm0             ;Z:   index = 0;
                                        ;     index |= ((b0[i] >> 7) << i);
    movups      xmm0, XMMWORD [block + 56 * SIZEOF_WORD]
                                        ;H: w0 = 56 57 58 59 60 61 62 63
...
%macro GET_SYM_AFTER 0
    ...
    pcmpeqw     mm_all_0xff, mm_all_0xff
                                        ;Z:   all_0xff[i] = 0xFF;
%endmacro
    ...
    pinsrw      xmm4, word [block + 59 * SIZEOF_WORD], 0
                                        ;G: w4 = 59 44 45 38 -- 39 46 --
    ...
    pinsrw      xmm4, word [block + 52 * SIZEOF_WORD], 1
                                        ;G: w4 = 59 52 45 38 -- 39 46 --
    movlps      xmm1, qword [block + 20 * SIZEOF_WORD]
                                        ;F: w1 = 20 21 22 23 37 44 51 58
    pinsrw      xmm4, word [block + 31 * SIZEOF_WORD], 4
                                        ;G: w4 = 59 52 45 38 31 39 46 --
    pshuflw     xmm1, xmm1, 01110010b   ;F: w1 = 22 20 23 21 37 44 51 58
    pinsrw      xmm4, word [block + 53 * SIZEOF_WORD], 7
                                        ;G: w4 = 59 52 45 38 31 39 46 53
                                        ;        (Row 6, offset 1)
    adc         code_temp, -1           ;Z:   code_temp += -1 + (code_temp >= 0 ? 1 : 0);
    pxor        xmm2, xmm2              ;G: w2[i] = 0;
    pcmpgtw     xmm0, xmm4              ;G: w0[i] = (w4[i] < 0 ? -1 : 0);
    pinsrw      xmm1, word [block + 15 * SIZEOF_WORD], 1
                                        ;F: w1 = 22 15 23 21 37 44 51 58
    paddw       xmm4, xmm0              ;G: w4[i] += w0[i];
    movaps      XMMWORD [t + 48 * SIZEOF_WORD], xmm4
                                        ;G: t[48 + i] = w4[i];
    movd        mm_temp, code_temp      ;Z:   temp = code_temp
    pinsrw      xmm1, word [block + 30 * SIZEOF_WORD], 3
                                        ;F: w1 = 22 15 23 30 37 44 51 58
                                        ;        (Row 5, offset 1)
    ...
    movaps      XMMWORD [t + (40 + 1) * SIZEOF_WORD], xmm1
                                        ;F: t[40 + i] = w1[i];
    pcmpeqw     xmm1, xmm0              ;F: w1[i] = (w1[i] == 0 ? -1 : 0);
    pinsrw      xmm5, word [block + 42 * SIZEOF_WORD], 0
                                        ;E: w5 = 42 49 56 57 50 51 58 59
    pinsrw      xmm5, word [block + 43 * SIZEOF_WORD], 5
                                        ;E: w5 = 42 49 56 57 50 43 58 59
    pinsrw      xmm5, word [block + 36 * SIZEOF_WORD], 6
                                        ;E: w5 = 42 49 56 57 50 43 36 59
    pinsrw      xmm5, word [block + 29 * SIZEOF_WORD], 7
                                        ;E: w5 = 42 49 56 57 50 43 36 29
                                        ;        (Row 4, offset 1)
%undef block
%define nbits  edx
%define nbitsb  dl
%define nbitsh  dh
    movzx       nbits, byte [NBITS(code_temp)]
                                        ;Z:   nbits = JPEG_NBITS(code_temp);
%undef code_temp
%define state  esi
...
%define sizeh  ch
    ...
    movaps      XMMWORD [t + (32 + 1) * SIZEOF_WORD], xmm5
                                        ;E: t[32 + i] = w5[i];
...
%endif
    ...
    movzx       nbits, byte [NBITS(nbits)]
                                        ;   nbits = JPEG_NBITS(nbits);
    lea         size, [size * 8 + nbits]
                                        ;   size = size * 8 + nbits;
    movd        mm_nbits, nbits         ;   nbits --> MMX register
...
.BEGIN:                                 ; .BEGIN:
    pand        mm_temp, [MASK_BITS(nbits)]
                                        ;   temp &= (1 << nbits) - 1;
    psllq       mm_code, mm_nbits       ;   code <<= nbits;
    add         nbits, size             ;   nbits += size;
    por         mm_code, mm_temp        ;   code |= temp;
    sub         free_bits, nbits        ;   if ((free_bits -= nbits) <= 0)
    jle         .EMIT_ERLOOP1           ;     insert code, flush buffer,
                                        ;     init size, goto .BLOOP1
...
; Round 2
; t points to the last used word, possibly below t_ if the previous index had
; 32 zero bits.
.ELOOP1:                                ; .ELOOP1:
    pmovmskb    size, xmm4              ; size = 0;
                                        ; size |= ((b4[i] >> 7) << i);
    pmovmskb    index, xmm5             ; index = 0;
                                        ; index |= ((b5[i] >> 7) << i);
    shl         size, 16                ; size <<= 16;
    ...
    lea         nbits, [t + (1 + DCTSIZE2) * SIZEOF_WORD]
                                        ; nbits = t + 1 + 64;
    and         nbits, -DCTSIZE2 * SIZEOF_WORD
                                        ; nbits &= -128;  /* now points to &t_[64] */
    sub         nbits, t                ; nbits -= t;
...
.ERLOOP2:                               ; .ERLOOP2:
    ...
    xor         size, size              ;   size = 0;  /* kill tzcnt input dependency */
    sub         free_bits, nbits        ;   if ((free_bits -= nbits) <= 0)
    jle         .EMIT_ERLOOP2           ;     insert code, flush buffer,
                                        ;     init size, goto .BLOOP2
    tzcnt       size, index             ;   size = # of trailing 0 bits in index
    movd        mm_nbits, nbits         ;   nbits --> MMX register
    psllq       mm_put_buffer, mm_nbits
                                        ;   put_buffer <<= nbits;
...
.ELOOP2:                                ; .ELOOP2:
    ...
    and         nbits, DCTSIZE2 * SIZEOF_WORD - 1
                                        ; nbits &= 127;
    and         t, -DCTSIZE2 * SIZEOF_WORD
                                        ; t &= -128;  /* t = &t_[0]; */
    cmp         nbits, (DCTSIZE2 - 2) * SIZEOF_WORD
                                        ; if (nbits != 62 * 2)
    je          .EFN                    ; {
    ...
    EMIT_QWORD  size, sizeb, sizeh, , , , , , .EFN
                                        ;     insert code, flush put_buffer
    align       16
